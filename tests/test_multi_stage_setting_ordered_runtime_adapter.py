...
def _observation(episode=1, *, failure=False):
    return OrderedScenarioObservation(
        ...
        prediction=ScenarioPrediction(
            ...
            pipeline_status="PIPELINE_FAILED" if failure else "COMPLETED",
            failed_stage="WORLD_STAGE2" if failure else None,
            failures=[{
...
def test_sealed_episode_with_candidate_failure_preserves_error_and_next_episode():
    from dataclasses import replace
    from evals.multi_stage_setting.contracts import PredictionBundleV3
    from evals.multi_stage_setting.evaluator import _runtime_failure_summary
    first = _observation(failure=True)
    first = replace(first, prediction=first.prediction.model_copy(update={
        "pipeline_status": ScenarioPipelineStatus.COMPLETED, "failed_stage": None,
    }))
    bundle = _build([first, _observation(2)], [_journal(), _journal(2)])
    restored = PredictionBundleV3.model_validate_json(bundle.model_dump_json())
    assert len(restored.scenarios) == 2
    assert all(row.runtime_state_trace.state_readiness == "SEALED" for row in restored.scenarios)
    assert restored.scenarios[0].failures == first.prediction.failures
    assert _runtime_failure_summary(restored)["total"] == 1
...
def test_real_prediction_journal_projection_and_scoring_preserve_mutation_history_without_gold_input(isolated_failure):
    ...
    if isolated_failure:
        bundle.scenarios[0].failures = _observation(failure=True).prediction.failures
    ...
    from evals.multi_stage_setting.evaluator import _runtime_failure_summary
    assert _runtime_failure_summary(bundle)["total"] == int(isolated_failure)
...
@pytest.mark.parametrize('mutation', ['events', 'backend_hash', 'output_state'])
def test_failed_ordered_trace_rejects_unsealed_effects(mutation):
    from evals.multi_stage_setting.contracts import PredictionBundleV3
    from evals.multi_stage_setting.state_effects import build_gold_state_chain
    from tests.test_multi_stage_setting_experimental_identity import _gold

    bundle = _build([_observation(failure=True)], [])
    trace = bundle.scenarios[0].runtime_state_trace
    if mutation == 'events':
        trace.applied_event_ids = ['unsealed-event']
    elif mutation == 'backend_hash':
        trace.backend_output_state_hash = 'different-output-hash'
    else:
        fact = next(f for t in build_gold_state_chain(_gold()).values() for f in t.after_state.world_facts)
        trace.output_state = EvaluationState(world_facts=[fact])
        trace.output_state_hash = trace.output_state.content_hash()
    with pytest.raises(ValueError, match='Failed ordered traces must preserve'):
        PredictionBundleV3.model_validate_json(bundle.model_dump_json())
