...
from evals.multi_stage_setting.contracts import (
    ...
    ScenarioPrediction,
    character_state_ref,
    WorldStage1Prediction, WorldStage2Prediction, world_state_ref, world_subject_ref,
)
...
def test_world_subject_and_property_targets_align_only_for_scoring(encoded):
    gold = _gold()
    chain = build_gold_state_chain(gold)
    expected = next(f for t in chain.values() for f in t.after_state.world_facts)
    raw_subject = "provisional-world:00000000-0000-4000-8000-000000000987"
    actual = expected.model_copy(update={
        "subject_ref": raw_subject,
        "ref": world_state_ref(expected.category, expected.subject_name, expected.scope_name,
                               expected.setting_name, subject_ref=raw_subject),
    })
    state = EvaluationState(world_facts=[actual])
    target = world_subject_ref(actual.category, actual.subject_name, subject_ref=raw_subject) if encoded else raw_subject
    bundle = PredictionBundleV3(
        fixture_hash=gold.fixture_hash, mode="COMMON_START", frozen_start_state=state,
        runtime_policy_version="common-start/v1", scenarios=[ScenarioPrediction(
            scenario_id="S1", runtime_state_trace=RuntimeStateTrace(
                input_state=state, output_state=state, input_state_hash=state.content_hash(),
                output_state_hash=state.content_hash(), source_hash="synthetic", elapsed_seconds=0,
            ), stage1=[WorldStage1Prediction(candidate_id="C1", domain="WORLD", category=actual.category,
                subject_name=actual.subject_name, setting_name="추가 설정", source_values=["합성 내용"])],
            stage2=[WorldStage2Prediction(source_candidate_id="C1", domain="WORLD", operation="ADD",
                consolidation_status="SINGLE", proposed_setting_name="추가 설정", proposed_value="합성 내용",
                target_ref=target)],
        )],
    )
    original = deepcopy(bundle)
    alignment = build_scoring_identity_alignment(gold, chain, bundle)
    assert raw_subject in alignment.worlds
    assert actual.ref in alignment.state_refs
    aligned = alignment.predictions(bundle)
    assert aligned.scenarios[0].stage2[0].target_ref == world_subject_ref(
        expected.category, expected.subject_name, subject_ref=expected.subject_ref)
    assert alignment.state_refs[actual.ref] == expected.ref
    assert bundle == original
    assert aligned.scenarios[0].runtime_state_trace == original.scenarios[0].runtime_state_trace
