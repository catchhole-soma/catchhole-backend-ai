...
def test_ordered_schema_failure_does_not_log_or_persist_rejected_values(caplog):
    source = candidate("C1", "profile.occupation")
    invalid = {**decision(source), "operation": "SYNTHETIC_PRIVATE_VALUE"}
    with caplog.at_level(logging.WARNING):
        result, requests = run([[invalid]], [source])
    assert isinstance(result, Exception)
    assert len(requests) == 3
    assert "SYNTHETIC_PRIVATE_VALUE" not in str(result)
    assert "SYNTHETIC_PRIVATE_VALUE" not in caplog.text
    assert "ValidationError" in str(result)
