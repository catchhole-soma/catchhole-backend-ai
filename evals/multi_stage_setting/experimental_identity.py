    ...
    for state in actual_states:
        ...
        for item in state.world_facts:
            ...
            if target is not None:
                # Subject-level ADD targets need the same unique alignment as
                # property refs; raw observations and trace hashes stay intact.
                aligned_ref = world_subject_ref(item.category, name, subject_ref=subject_ref)
                state_refs[item.subject_ref] = aligned_ref
                state_refs[world_subject_ref(item.category, item.subject_name,
                                             subject_ref=item.subject_ref)] = aligned_ref
