        ...
        if experimental:
            ...
            if self.mode == EvaluationMode.ORDERED_PROVISIONAL:
                ...
                for item in self.scenarios:
                    ...
                    failed = item.pipeline_status != "COMPLETED" or item.execution_failure is not None
                    ...
                    if failed and (
                        trace.input_state != trace.output_state
                        or trace.backend_input_state_hash != trace.backend_output_state_hash
                        or trace.applied_event_ids
                    ):
                        raise ValueError("Failed ordered traces must preserve input state and hashes without applied events.")
