...
def build_ordered_prediction_bundle(
    *,
    ...
    Candidate errors remain in scoring denominators. A completed episode can
    retain isolated errors when its Backend journal is sealed. An interrupted
    episode has no sealed journal and no following observation may proceed.
    """
    if not observations:
        raise ValueError("At least one observed episode is required.")
    failed = [
        index for index, observation in enumerate(observations)
        if observation.prediction.pipeline_status != ScenarioPipelineStatus.COMPLETED
        or observation.prediction.execution_failure is not None
    ]
