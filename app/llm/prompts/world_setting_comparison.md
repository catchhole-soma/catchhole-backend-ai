...
- 기존 속성과 실질적으로 중복되어 EXCLUDE한다면 target_ref와 기존 properties에 실제 존재하는 matched_property_name을 반드시 반환한다. comparison_reason에서 특정 기존 속성을 비교 대상으로 언급할 때도 이 두 필드를 생략하지 않는다.
- 일시적 사건·현재 상태·세계관 설정으로 부적절한 근거처럼 특정 기존 속성과 비교하지 않고 EXCLUDE한다면 matched_property_name은 null로 반환한다.
- EXCLUDE도 검토 화면에서 추출값을 보존할 수 있도록 proposed_setting_name은 후보의 setting_name을 유지한다. extracted_values가 하나면 proposed_value를 그대로 유지하고, 여러 개면 MERGED일 때 모든 보완 정보를 자연스럽게 합치며 CONFLICT일 때 candidate.extracted_value를 그대로 유지한다.
