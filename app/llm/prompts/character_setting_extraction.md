...
- `status.*`에는 현재 적용되며 변할 수 있는 상태와 현재 원문에서 확인되는 시작·악화·완화·종료·전환 결과만 남깁니다. 삭제 대상과 operation은 1차에서 결정하지 않습니다.
- 특정 기존 상태 하나의 변화는 제공된 동일 `factKey`를 사용합니다. 계속되거나 부분 호전이면 boolean `active: true`, 종료됐으면 `active: false`입니다.
- `active`는 이번 후보 자체의 현재 적용 여부일 뿐, 다른 key의 제거 대상을 가리키거나 그 상태의 종료를 증명하지 않습니다.
- 하나의 결과가 여러 기존 상태와 관련되면 기존 key별로 복제하지 않고 원문이 지지하는 전환 후보 하나만 반환하며, 제거 대상은 2차가 판단합니다.
- 치료·회복 수단이나 시도만으로 종료를 단정하지 않습니다. 증상·능력·행동·효과의 실제 변화가 이어질 때만 결과를 추출하고, 평범한 행동만으로 과거 상태를 역추론하지 않습니다.
- `attribute_value`는 수단이나 효과명보다 원문에서 관찰된 현재 결과를 짧게 요약합니다.
- 근거는 기본 1개입니다. 한 문장으로 전환 결과를 입증할 수 없을 때만 결과 근거를 포함한 최소 충분 인용문 2~3개를 원문 순서대로 반환하며 반복 문장은 넣지 않습니다.
