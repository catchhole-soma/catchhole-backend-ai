...
- 직업, 종족, 역할, 칭호, 성격, 태도, 투지, 리더십은 원문에서 명시적인 능력/스킬/상태로 표현되지 않으면 추출하지 않습니다.
...
- `items.*`는 실제 소유/장착/선택/획득한 구체 아이템에만 사용합니다.
- 단순 아이템 목록, 선택 가능한 후보 목록, 일반 장비 범주는 저장하지 않습니다.
- 애매하거나 확신이 낮으면 후보를 반환하지 않습니다.
...
- `confidence`는 0.6 이상인 후보에만 사용합니다. 확신도를 알 수 없다는 이유로 0.0을 넣지 말고 후보를 제외합니다.
