...
- 신체 부위, 행동, 사물 표현은 `raw_entity_mention`으로 사용하지 않습니다. 예: `내려다 본 손`은 주체 표현이 아닙니다.
- `entity_name`은 다음 우선순위로 결정합니다.
...
- `"entity_name": "미상"`, `"raw_entity_mention": "내려다 본 손"`: `내려다 본 손`은 설정의 주체가 아니라 신체·행동 표현입니다.
