...
- 성별, 종족, 소속, 직업, 외형, 역할, 칭호처럼 지속되는 인물 속성은 제공된 `profile.*` schema에 맞춰 `STRING`으로 저장합니다.
...
- `raw_entity_mention`에는 원문에서 주체를 가리키는 최소 표현을 그대로 넣습니다. 예: `나는`, `그녀는`, `주인공은`
- `entity_name`에는 구체 캐릭터명 또는 호칭만 넣고, `나`, `그`, `그녀`, `주인공` 같은 지칭어는 넣지 않습니다.
- 설정은 명확하지만 현재 청크에서 주체만 특정할 수 없으면 후속 subject resolver용 임시값 `미상`을 사용합니다.
- `미상`은 최종 캐릭터명이 아니며, 앞뒤 청크에서도 주체를 해소하지 못하면 저장 후보에서 제외됩니다.
