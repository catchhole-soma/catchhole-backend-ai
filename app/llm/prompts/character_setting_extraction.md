...
- 같은 상태의 시작·악화·완화는 단계 표현마다 새 key를 만들지 말고 안정적인 상태 명사구를 유지하며, 강도와 진행 정도는 값에 적습니다(예: `status.생명력_위험`, 금지: `status.체력_5%_상태`). 신체 부위·원인·효과가 달라 별도 현재 상태로 추적해야 할 때만 key를 나눕니다.
