...
- `setting_candidate.py`
  ...
  - 캐릭터 Fact 2차 비교 컬럼은 Spring Flyway 스키마를 그대로 mirror합니다. Python 직접 저장 시에는 발견 후보를 `NOT_REQUIRED`, 모호한 주체를 `WAITING_FOR_CHARACTER_MATCH`, 기존 캐릭터와 구체적 신규 이름의 설정 후보를 `PENDING`으로 초기화합니다.
