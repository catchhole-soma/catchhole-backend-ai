...
class CharacterFactComparisonWorker:
    """사용자 수정 등으로 생성된 단일 후보 재비교 Job을 동시성 1로 처리한다."""

    def __init__(
        self,
        spring_client: SpringWorkerApi | None = None,
        comparison_pipeline: CharacterFactComparisonPipeline | None = None,
    ...
    async def aclose(self) -> None:
        ...
        for resource_name, close_operation in close_operations:
            try:
                ...
                logger.exception(
                    "Failed to close character comparison Worker resource. resource=%s",
                    resource_name,
                )
    ...
    async def run_once(self) -> WorkerRunResult:
        ...
        if payload is None:
            return WorkerRunResult(
                ...
                message="Claimable character-fact comparison job does not exist.",
            )
    ...
    async def process_claimed(self, payload: WorkerAnalysisJobPayload) -> WorkerRunResult:
        ...
        try:
            ...
            await self.spring_client.report_progress(
                ...
                payload.lease_token,
                AnalysisStep.CHARACTER_FACT_COMPARISON.value,
            )
            ...
            try:
                ...
                logger.exception(
                    "Failed to report character comparison job failure. analysis_job_id=%s",
                    payload.analysis_job_id,
                )
            raise
        ...
        return WorkerRunResult(
            ...
            message="Character-fact comparison job completed.",
    ...
    def _validate_payload(self, payload: WorkerAnalysisJobPayload) -> None:
        ...
        if payload.setting_candidate_id is None:
            raise ValueError("Character comparison job must include settingCandidateId.")
