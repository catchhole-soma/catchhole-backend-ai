...
현재 작업은 `knownCharacters` 기반 기본 매칭까지만 처리합니다. `나`, `그`, `그녀`, `주인공` 같은 지칭어라도 `entity_name`이 기존 캐릭터 1명과 유일하게 매칭되면 `MATCHED`로 저장합니다.
...
- fallback을 적용한다면 `entity_name="미상"` 또는 지칭어 같은 placeholder 후보를 previous/current/next chunk 문맥으로 해소한 뒤 일반 매칭 로직으로 넘길지 결정해야 합니다.
