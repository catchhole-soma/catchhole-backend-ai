...
API 호출은 DB 세션을 열기 전에 수행하며, 임베딩 필드 갱신은 회차 단위의 짧은 트랜잭션으로 처리합니다. 현재는 API 호출이나 DB 갱신 중 발생한 예외를 모두 임베딩 실패로 집계하고 설정 후보 추출을 계속합니다. 따라서 분석 작업은 성공할 수 있지만 갱신이 commit되지 않은 청크의 `embedding`은 `NULL`일 수 있으며, 이 청크는 backfill 대상입니다.

이 정책은 외부 API의 일시적 실패를 설정 추출과 분리하기 위한 현재 구현입니다. 중복·누락된 chunk ID처럼 데이터 정합성 문제를 나타내는 예외까지 계속 처리하는 것은 안전하지 않으므로, 임베딩 예외 유형을 구분하고 정합성 예외는 분석 작업에 전파하는 보완이 남아 있습니다.
...
- OpenAI Embeddings Client도 임베딩 입력 token usage를 `EmbeddingBatchResponse`와 `ChunkEmbeddingResult`에 담습니다.
...
- `NVM-141`: 신규 `episode_chunks` 임베딩 생성·저장은 연결되었습니다. 범용 pgvector Top-K 검색, 기본 필터, 검색 품질 검증과 기존 청크 backfill이 남아 있습니다.
