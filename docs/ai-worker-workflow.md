...
flowchart TD
    ...
    AC --> AE["save_items에 후보 추가"]
    AD --> AE

    AE --> AF{"모든 episode/chunk 처리 완료?"}
    AF -- "아니오" --> J
...
11. `app/analysis/character_name_resolver.py`
    - `raw_entity_mention`, `entity_name`, `knownCharacters`로 기존 캐릭터 매칭 상태를 계산하는 흐름을 봅니다.
12. `app/services/setting_candidate_service.py`
    - 검증된 추출 결과가 `setting_candidates`로 저장되는 흐름을 봅니다.
13. `app/mappers/setting_candidate_mapper.py`
    - LLM 후보와 매칭 결과가 DB 모델 필드로 어떻게 옮겨지는지 봅니다.

## adjacent chunk fallback 적용 시 변경 지점

현재 작업은 `knownCharacters` 기반 기본 매칭까지만 처리하고, `나`, `그`, `그녀`, `주인공` 같은 지칭어 후보는 보수적으로 `AMBIGUOUS`로 남깁니다.

현재 흐름:

```text
raw_entity_mention이 지칭어
-> character_name_resolver.py에서 즉시 AMBIGUOUS
```
...
- `NVM-225`: 지칭어 후보의 adjacent chunk fallback resolver 적용 여부를 결정합니다.
- fallback을 적용한다면 `character_name_resolver.py`의 "raw 지칭어면 즉시 `AMBIGUOUS`" 분기를 분리해, context-resolved 후보가 일반 매칭 로직으로 이어질 수 있게 해야 합니다.
