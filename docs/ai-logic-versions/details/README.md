...
[사람용 목록](../README.md) · [요약 템플릿](../TEMPLATE.md) · [상세 템플릿](TEMPLATE.md)

아래 파일 경로는 `docs/ai-logic-versions/` 기준이다.

1. `details/vNNNN.md`에 근거를 먼저 기록하고 `vNNNN.md`의 최신 요약과 목록을 갱신한다. 두 파일은 같은 변경에 포함한다.
2. 사람용 요약은 20~35줄 안팎으로 쓴다. 변경 이유·점수·남은 문제를 우선하고, SHA·해시·토큰 사용량·재현 명령·과거 실행·반복 지침은 상세에 둔다.
3. 수치와 조건의 기준은 상세 기록이다. 요약은 여기서 옮기며 다른 수치를 따로 관리하지 않는다. 의미 판정 미완료의 `null`은 요약에도 `null`, 미측정은 `—`로 표시한다.
4. 오류·미판정·미측정 사유와 필요한 다음 조치는 요약에도 남긴다. Actions 성공만으로 완료나 개선을 선언하지 않는다.
5. 파일을 나누거나 요약을 갱신해도 과거 실행의 수치·조건·복원 근거는 상세에 보존한다. 원문·Gold·개별 예측·비밀값은 어느 쪽에도 복사하지 않는다.
...
- main의 마지막 번호에 1을 더해 `v0002`, `v0004`처럼 부여한다. 병렬 PR은 머지 전에 최신 목록과 기준 코드를 확인해 번호 충돌을 해소한다. 기준 코드가 달라지면 재평가하거나 미측정 사유를 갱신한다.
- 같은 버전의 추가 실험은 해당 상세 MD에 실행 단위를 덧붙인다. 이미 기록한 실행·실패를 덮어쓰지 않고 정정은 사유를 남긴다.
- 문서·테스트만 바뀌거나 동작을 보존하는 정리는 버전을 올리지 않고 PR에 사유를 적는다. 채점기·Gold·judge만 바뀌면 제품 로직 버전을 유지하고 같은 상세 MD에 별도 평가를 추가한다.
- 이 번호는 AI 전체 이력의 이름이다. 기존 프롬프트 cache key도 실제 값으로 기록하고, 프롬프트 변경 시 해당 key를 함께 갱신한다.

## 상세 기록에 남길 최소 근거

1. 이전 버전과 바뀐 단계·파일·근거 커밋을 연결한다. 사람용 요약에는 이번에 새로 바뀐 동작과 이유만 3개 안팎으로 적고, 기존 기능의 출처는 필요할 때 상세에 보완한다.
...
run URL만 남기지 말고 **실제 조건과 집계 숫자도 상세 MD에 보관**한다.
...
상세의 다단계 점수는 도메인별로 아래 값을 기록한다. 비율은 원본의 0~1 단위를 유지한다.
사람용 표에는 1차 추출 F1·최종 상태 F1을 백분율로 소수점 둘째 자리까지 표시한다.
2차 정확도를 요약에 추가한다면 실제 채점 수/전체 정답 수도 함께 표시한다.
...
1. 대상 버전의 상세 기록에서 복원 SHA와 실행 설정을 확인한다. 필요하면 `git worktree add --detach <별도-경로> <전체-SHA>`로 이전 코드를 따로 열어 현재 코드와 비교한다.
