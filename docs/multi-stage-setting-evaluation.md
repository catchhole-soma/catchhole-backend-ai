...
평가를 실행한 뒤에는 [AI 로직 버전 기록](ai-logic-versions/README.md)에 실제 코드·채점기
커밋, 고정 입력과 모델·judge 조건, 핵심 점수·미판정·실패 수를 남깁니다. Actions 결과 링크와
함께 집계값을 보관하고, 조건이 다른 실행이나 미완료 점수를 개선 근거로 사용하지 않습니다.
...
Actions의 `extraction_max_output_tokens`는 캐릭터 1차 추출 상한만 6,000(기본) 또는 12,000으로 선택한다.
기존 절단 재시도 상한은 12,000이므로 12,000 실험에서는 더 큰 상한으로 확장하지 않는다.
이 값은 예측 step에만 주입하고 운영 기본 설정·주체 해소·2차 비교·judge 상한은 변경하지 않는다.
출력 상한 증가와 HTTP 읽기 제한(120초)은 별개이므로 저장된 출력량과 네트워크 예외를 함께 확인한다.
