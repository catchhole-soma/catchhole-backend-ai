...
요청한 각 회차에는 Notion 정답 행이 하나 이상 있어야 하며 일부 회차가 비어 있으면 실행을
실패시킵니다. 구조화 DB에 실제 회차 제목 컬럼이 없으므로 임의의 `N화` 제목을 만들지 않고 제목
메타데이터 없이 분석합니다. Summary와 보관 artifact에는 분석 모델과 Semantic Judge 사용 여부·
모델을 함께 기록하여 서로 다른 실행 결과를 구분할 수 있게 합니다.
